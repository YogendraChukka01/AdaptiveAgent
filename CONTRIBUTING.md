...
Before installing Python dependencies, ensure these system packages are installed:
...
# macOS
brew install postgresql  # provides libpq

# Arch Linux
sudo pacman -S postgresql-libs
```

> `libpq-dev` is required by `psycopg` (the PostgreSQL async driver). Without it,
> `pip install` will fail with a compiler error.
